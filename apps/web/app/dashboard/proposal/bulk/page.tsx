...
import { ClientFormData, industries as validIndustries, timelineOptions } from '@/lib/schemas'
...
type ClientSpreadsheetRow = Partial<Record<
  'clientName' | 'clientEmail' | 'clientPhone' | 'businessName' | 'industry' | 'services' | 'timeline' | 'currentWebsite' | 'specialRequirements',
  string
>>

const normalizeIndustry = (value?: string): ClientFormData['industry'] =>
  validIndustries.includes(value as ClientFormData['industry']) ? value as ClientFormData['industry'] : 'doctors'

const normalizeTimeline = (value?: string): ClientFormData['timeline'] =>
  timelineOptions.includes(value as ClientFormData['timeline']) ? value as ClientFormData['timeline'] : '1-2months'
...
export default function BulkProposalPage() {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const jsonData = XLSX.utils.sheet_to_json<ClientSpreadsheetRow>(worksheet)
  ...
  const generateAll = async () => {
    ...
    for (let i = 0; i < clients.length; i++) {
      ...
      try {
        ...
        console.error('Bulk proposal generation failed:', error)
        setClients(prev => prev.map((c, idx) =>
  ...
  const updateEditingClientField = (field: keyof ClientFormData, value: ClientFormData[keyof ClientFormData]) => {
    if (editingClient) {
      setEditingClient({
        ...editingClient,
        data: {
  ...
  return (
    <DashboardLayout>
      <div className="max-w-7xl mx-auto">
        <div className="mb-8">
          <p className="studio-label">Proposals</p>
          <h1 className="text-2xl font-bold text-[#EDE9DC]">Bulk Proposal Generator</h1>
          <p className="text-sm text-[#9E9880] mt-1">
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <p className="text-slate-400 text-sm mb-4">
                        Customize the services you will provide to this client
                      </p>
