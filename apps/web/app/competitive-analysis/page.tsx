    ...
    <div className="container mx-auto px-4 py-8">
      <div className="text-center mb-12">
        ...
        <p className="mx-auto mt-4 max-w-2xl text-sm text-slate-500">
          This tool does not perform live competitor research. Treat built-in examples as strategy frameworks and verify facts with current sources.
        </p>
      </div>
                  ...
                  {customCompetitors.map((competitor, i) => (
                    <div key={i} className="border rounded-lg p-4">
                      ...
                      <div className="grid md:grid-cols-4 gap-2 text-sm">
                        <div><span className="text-slate-400">Industry:</span> {industries.find(i => i.value === competitor.industry)?.label}</div>
                        <div><span className="text-slate-400">Stage:</span> {fundingStages.find(s => s.value === competitor.fundingStage)?.label}</div>
                        <div><span className="text-slate-400">Size:</span> {employeeCounts.find(c => c.value === competitor.employeeCount)?.label}</div>
                        <div><span className="text-slate-400">Model:</span> {businessModels.find(m => m.value === competitor.businessModel)?.label}</div>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                {marketPositions.map((position, i) => (
                  <div key={i} className="border rounded-lg p-4">
                    ...
                    <div className="grid md:grid-cols-2 gap-4">
                      ...
                      <div>
                        <div className="mb-2">
                          <span className="text-sm text-slate-400">Target Segment:</span>
                          <p className="text-sm">{position.targetSegment}</p>
                        </div>
                        <div>
                          <span className="text-sm text-slate-400">Competitive Advantage:</span>
                  ...
                  <AccordionItem key={i} value={`category-${i}`}>
                    ...
                    <AccordionContent className="space-y-4">
                      {category.tools.map((tool, j) => (
                        <div key={j} className="border rounded-lg p-4 bg-slate-900">
                          <div className="flex justify-between items-start mb-2">
              ...
              {strategyRecommendations.map((strategy, i) => (
                <div key={i} className="border rounded-lg p-4">
                  ...
                  <div className="grid md:grid-cols-2 gap-4">
                    ...
                    <div>
                      <h5 className="font-medium text-sm text-slate-400 mb-2">Timeline:</h5>
              ...
              {competitors.map((competitor) => (
                <div key={competitor.id} className="border rounded-lg p-4">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <a href={`https://${competitor.website}`} target="_blank" rel="noopener noreferrer" className="text-sm text-slate-400 hover:text-blue-600">
                      {competitor.website}
                    </a>
                  </div>
                  <div className="grid md:grid-cols-3 gap-4">
                    <div>
                      ...
                      <span className="text-xs text-slate-400">{competitor.seoScore}/100</span>
                    </div>
                    <div>
                      <Label>Content Score</Label>
                      <Progress value={competitor.contentScore} className="h-3 mt-1" />
                      <span className="text-xs text-slate-400">{competitor.contentScore}/100</span>
                    </div>
                    <div>
                      <Label>Social Media Score</Label>
                      <Progress value={competitor.socialScore} className="h-3 mt-1" />
                      <span className="text-xs text-slate-400">{competitor.socialScore}/100</span>
                    </div>
                  </div>
