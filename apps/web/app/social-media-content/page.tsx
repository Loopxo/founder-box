  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="text-center mb-12">
        <p className="studio-label">Founder Tools</p>
        <h1 className="text-2xl font-bold text-[#EDE9DC]">
          Advanced Social Media Growth System
        </h1>
        <p className="text-sm text-[#9E9880] mt-1 max-w-3xl mx-auto mb-6">
                    ...
                    <AccordionContent className="space-y-4">
                      {category.ideas.map((idea, j) => (
                        <div key={j} className="border rounded-lg p-4">
                          ...
                          <p className="text-sm text-slate-400 mt-1 mb-2">{idea.description}</p>
                  ...
                  <div key={i}>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {category.hashtags.map(tag => (
                        <div key={tag} className="flex items-center gap-1 bg-slate-800 rounded-full px-2 py-1">
                    ...
                    <AccordionContent className="space-y-4">
                      {category.tactics.map((tactic, j) => (
                        <div key={j} className="border rounded-lg p-4 bg-gradient-to-r from-blue-50 to-purple-50">
                          ...
                          <div className="grid md:grid-cols-2 gap-4">
                            <div>
                              <h5 className="font-medium text-sm text-slate-400 mb-1">Implementation:</h5>
                              <p className="text-sm">{tactic.implementation}</p>
                            </div>
                            <div>
                              <h5 className="font-medium text-sm text-slate-400 mb-1">Expected Results:</h5>
                ...
                <div key={i} className="border rounded-lg p-4">
                  ...
                  <div className="grid gap-4">
                    {platform.strategies.map((strategy, j) => (
                      <div key={j} className="bg-slate-900 rounded-lg p-4">
                        <h4 className="font-medium mb-2">{strategy.name}</h4>
                        <p className="text-sm text-slate-400 mb-2">{strategy.description}</p>
                        <div className="grid md:grid-cols-2 gap-4">
                          <div>
                            <h5 className="font-medium text-sm text-slate-400 mb-1">How to implement:</h5>
                            <p className="text-sm">{strategy.implementation}</p>
                          </div>
                          <div>
                            <h5 className="font-medium text-sm text-slate-400 mb-1">Performance boost:</h5>
            ...
            <CardContent className="space-y-6">
              {advancedAnalytics.map((metric, i) => (
                <div key={i} className="border rounded-lg p-4 bg-gradient-to-r from-blue-50 to-indigo-50">
                  ...
                  <p className="text-slate-300 mb-3">{metric.description}</p>
                  <div className="grid md:grid-cols-3 gap-4 mb-4">
                    <div>
                      <h5 className="font-medium text-sm text-slate-400 mb-1">Calculation:</h5>
                      <code className="text-sm bg-slate-800 px-2 py-1 rounded">{metric.calculation}</code>
                    </div>
                    <div>
                      <h5 className="font-medium text-sm text-slate-400 mb-1">Benchmarks:</h5>
                      <p className="text-sm font-medium">{metric.benchmarks}</p>
                    </div>
                    <div>
                      <h5 className="font-medium text-sm text-slate-400 mb-1">Platform Relevance:</h5>
                      <Badge variant="secondary">All Platforms</Badge>
                    </div>
                  </div>
                  <div>
                    <h5 className="font-medium text-sm text-slate-400 mb-2">Actionable Insights:</h5>
                    <ul className="list-disc list-inside space-y-1 text-sm">
            ...
            <CardContent className="space-y-6">
              {competitorAnalysisFramework.map((strategy, i) => (
                <div key={i} className="border rounded-lg p-4">
                  ...
                  <div className="grid md:grid-cols-2 gap-4">
                    <div>
                      <h5 className="font-medium text-sm text-slate-400 mb-2">Tools Needed:</h5>
                      <div className="flex flex-wrap gap-2">
                    ...
                    <div>
                      <h5 className="font-medium text-sm text-slate-400 mb-2">Implementation Steps:</h5>
            ...
            <CardContent className="space-y-6">
              {crisisManagementStrategies.map((crisis, i) => (
                <div key={i} className="border rounded-lg p-4 bg-gradient-to-r from-orange-50 to-red-50">
                  ...
                  <div className="mb-4">
                    <h5 className="font-medium text-sm text-slate-400 mb-1">Framework:</h5>
                    <p className="text-sm font-mono bg-slate-800 px-2 py-1 rounded">{crisis.response_framework}</p>
                  </div>
                  <div className="grid md:grid-cols-2 gap-4 mb-4">
                    <div>
                      <h5 className="font-medium text-sm text-slate-400 mb-2">Immediate Actions (First 24 hours):</h5>
                      <ul className="list-disc list-inside space-y-1 text-sm">
                        {crisis.immediate_actions.map((action, j) => <li key={j}>{action}</li>)}
                      </ul>
                    </div>
                    <div>
                      <h5 className="font-medium text-sm text-slate-400 mb-2">Long-term Strategy:</h5>
                      <ul className="list-disc list-inside space-y-1 text-sm">
                  ...
                  <div>
                    <div className="flex justify-between items-center mb-2">
                      <h5 className="font-medium text-sm text-slate-400">Example Response Template:</h5>
                      <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => copyToClipboard(crisis.example_response)}>
                        <Copy className="h-4 w-4" />
                      </Button>
                    </div>
                    <p className="text-sm bg-slate-800 border rounded p-3 italic">{crisis.example_response}</p>
                  </div>
                </div>
              ))}
            </CardContent>
                    ...
                    <div className="space-y-3">
                        {engagementTactics.responseTemplates.map((template, i) => (
                            <div key={i} className="border rounded-lg p-3">
                                ...
                                <p className="text-sm text-slate-400 mt-1">{template.template}</p>
                            </div>
                        ))}
                    </div>
                </div>
