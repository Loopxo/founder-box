...
export async function POST(request: NextRequest) {
  ...
  const rateLimitResponse = await enforceApiRateLimit({ ip: meta.ipAddress, scope: "mobile:auth:request-otp", limit: 10 })
  ...
  if (dayCount >= 10) {
    await flagSuspiciousUsage({ type: "mobile_otp_request_burst", reason: "Too many mobile OTP requests in 24 hours.", severity: "medium", metadata: { email } })
  ...
  if (resendKey && from) {
    ...
    const { error } = await resend.emails.send({
      from,
      to: email,
      subject: "Your FounderBox mobile login code",
      text: `Your FounderBox login code is ${code}. It expires in 10 minutes.`,
    })

    if (error) {
      await writeAuditEvent({ action: "mobile.auth.otp.request_failed", userId: user.id, metadata: { email, provider: "resend" }, request })
      return NextResponse.json({ error: "Could not send OTP email. Check Resend configuration and try again." }, { status: 502 })
    }
  }

  await writeAuditEvent({ action: "mobile.auth.otp.requested", userId: user.id, metadata: { email }, request })
