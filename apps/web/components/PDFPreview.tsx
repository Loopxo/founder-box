...
interface ClientPreviewData {
  ...
  customPricing?: PricingPackagePreview[]
}

interface PricingPackagePreview {
  name?: string
  price?: string
  features?: string[]
}

export interface ProposalTemplatePreview {
  problems?: Array<{ title?: string; description?: string }>
  solutions?: Array<{ title?: string; description?: string; benefits?: string[] }>
  caseStudy?: { title?: string; description?: string; results?: string[] }
  pricing?: PricingPackagePreview[]
}

interface PDFPreviewProps {
  clientData: ClientPreviewData
  template: ProposalTemplatePreview
  theme: ThemeConfig
  agencyConfig: AgencyConfig
  isOpen: boolean
  onClose: () => void
...
<img 
        ...
        <div className="grid grid-cols-2 gap-8 text-left max-w-2xl mx-auto">
          ...
          <div>
            ...
            <p>Services: {Array.isArray(clientData.services) ? clientData.services.length : 0} selected</p>
          </div>
        </div>
      </div>
    </div>
  ...
  const renderIndustryNeeds = () => renderContentPage(
    'industry-needs',
    `What ${clientData.industry ? clientData.industry.charAt(0).toUpperCase() + clientData.industry.slice(1) : 'Business'}s Need Today`,
